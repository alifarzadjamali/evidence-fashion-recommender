...
def build_no_rag_prompt(case: Mapping[str, Any], word_limit: int | None = None) -> str:
    ...
    instruction = (
        "Write a 45–75 word explanation, aiming for about 65 words in 2–3 sentences. Explain "
    ...
    if word_limit is not None:
        instruction = (
            f"Write a recommendation explanation within the 45–75 word shared contract. "
            f"{instruction}"
        )
    return (
        f"{instruction}\n\n"
...
def build_rule_rag_prompt(
    ...
    prompt = (
        "Write an evidence-grounded recommendation explanation in 45–75 words, aiming for "
